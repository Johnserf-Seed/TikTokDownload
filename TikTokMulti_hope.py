            ...
            try:
                if self.musicarg == "yes":                              # 保留音频
                    ...
                    if music.status_code == 200:                        # 判断是否响应成功
                        ...
                        if self.mode == 'post':
                            m_url = self.save + '/' + re.sub(
                                r'[\\/:*?"<>|\r\n]+', "_", music_title) + '_' + author_list[i] + '.mp3'
                        else:
                            m_url = self.save + '/' + str(self.like_counts)+ '、' + re.sub(
                                r'[\\/:*?"<>|\r\n]+', "_", music_title) + '_' + author_list[i] + '.mp3'
            ...
            try:
                ...
                try:
                    if video.status_code == 200:                        # 判断是否响应成功
                        ...
                        if self.mode == 'post':
                            v_url = self.save + '/' + re.sub(
                                r'[\\/:*?"<>|\r\n] + ', "_", author_list[i]) + '.mp4'
                        else:
                            v_url = self.save + '/' + str(self.like_counts)+ '、' + re.sub(
                                r'[\\/:*?"<>|\r\n] + ', "_", author_list[i]) + '.mp4'
