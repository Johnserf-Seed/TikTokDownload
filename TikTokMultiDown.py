  ...
  ╚══██╔══╝██║██║ ██╔╝╚══██╔══╝██╔═══██╗██║ ██╔╝██╔══██╗██╔═══██╗██║    ██║████╗  ██║██║     ██╔═══██╗██╔══██╗██╔══██╗
    ...
    def video_info(self, result, max_cursor):
        ...
        self.videos_download(author_list, video_list,
                             uri_list, aweme_id, nickname, max_cursor)
        ...
        for i in range(len(author_list)):
            ...
            if len(author_list[i]) > 182:
                ...
                print("[  提示  ]:", "截取后的文案：{0}，长度：{1}".format(
                    author_list[i], len(author_list[i])))
            ...
            # 尝试下载视频
            try:
                new_video_list.append(uri_url %
                                      uri_list[i])            # 生成1080p视频链接
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument('--count', '-c', type=int,
                            help='单页下载的数量，默认参数 35 无须修改', default=35)
