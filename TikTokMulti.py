...
class TikTok():
    ...
    """
        ...
        if os.path.isfile("conf.ini") == True:
            ...
            try:
                ...
                r = requests.get(
                    'https://gitee.com/johnserfseed/TikTokDownload/raw/main/conf.ini')
  ...
  ╚══██╔══╝██║██║ ██╔╝╚══██╔══╝██╔═══██╗██║ ██╔╝██╔══██╗██╔═══██╗██║    ██║████╗  ██║██║     ██╔═══██╗██╔══██╗██╔══██╗
    ...
    def judge_link(self):
        ...
        DbTool.createTable(self.nickname)
    ...
    def video_info(self, result, max_cursor):
        ...
        for i2 in range(self.count):
            try:
                ...
                videoinfo.append(str(result[i2]['statistics']))
                # dynamic_cover.append(str(result[i2]['video']['dynamic_cover']['url_list'][0]))
            except Exception as error:
                # print(error)
                pass
        # self.videos_download(author_list,video_list,aweme_id,nickname,videoinfo,max_cursor)
        # return self,author_list,video_list,aweme_id,nickname,videoinfo,max_cursor
        self.videos_download(author_list, video_list,
                             aweme_id, nickname, videoinfo, max_cursor)
        return self, author_list, video_list, aweme_id, nickname, videoinfo, max_cursor
    ...
    # 音视频下载
    def videos_download(self, author_list, video_list, aweme_id, nickname, videoinfo, max_cursor):
        # 创建并检测下载目录是否存在
        try:
        ...
        for i in range(self.count):
            ...
            if len(videoinfo) == 0 or i > len(videoinfo)-1:
                break
            ...
            try:
                ...
                creat_time = time.strftime(
                    "%Y-%m-%d %H.%M.%S", time.localtime(js['item_list'][0]['create_time']))
            except Exception as error:
                print("网络请求出错 跳过当前视频：{0}".format(error))
                pass
            ...
            DbTool.insertDate(eval(videoinfo[i]), author_list[i], creat_time)

            if len(author_list[i]) > 182:
                print("-----> 文件名称太长 进行截取：{0}，{1}，长度：{2}".format(
                    videoinfo[i], author_list[i], len(author_list[i])))
                author_list[i] = author_list[i][0:180]
                print(
                    "-----> 截取后的文字：{0}，长度：{1}".format(author_list[i], len(author_list[i])))

            # 每次判断视频是否已经下载过
            try:
                if creat_time + author_list[i] + '.mp4' in v_info:
                    print('[  提示  ]:'+author_list[i] +
                          '[文件已存在，为您跳过]', end="")  # 开始下载，显示下载文件大小
                    for i in range(20):
                ...
                if self.musicarg == "yes":  # 保留音频
                    ...
                    if music.status_code == 200:  # 判断是否响应成功
                        ...
                        if self.mode == 'post':
                            ...
                            m_url = self.save + self.mode + "/" + self.nickname + '/' + \
                                str(self.like_counts) + '、' + re.sub(
                                    r'[\\/:*?"<>|\r\n]+', "_", music_title) + '_' + author_list[i] + '.mp3'
                    ...
                    if video.status_code == 200:  # 判断是否响应成功
                        ...
                        if self.mode == 'post':
                            v_url = self.save + self.mode + "/" + \
                                nickname[i] + '/' + creat_time + \
                                re.sub(r'[\\/:*?"<>|\r\n]+', "_",
                                       author_list[i]) + '.mp4'
                        else:
                            v_url = self.save + self.mode + "/" + self.nickname + '/' + \
                                str(self.like_counts) + '、' + \
                                re.sub(r'[\\/:*?"<>|\r\n]+', "_",
                                       author_list[i]) + '.mp4'
