...
class TikTok():
    ...
    def single_down(self,url):
        ...
        try:
            #创建并检测下载目录是否存在
            os.makedirs('.\\Download\\' + nickname)
        except:
            pass
        print('视频',video_title,'   下载中')
        with open(nickname +  "\\" + video_title +'.mp4','wb') as f:
            f.write(r.content)

        #原声下载    
        r=requests.get(url=music_url,headers=headers)
        print('音频',music_title,'    下载中')
        with open(nickname +  "\\" + music_title + '.mp3','wb') as f:
            f.write(r.content)
        input('....下载完成，按任意键退出....\r')
        sys.exit()
        return 
    ...
    #返回个人主页api数据
    def get_info(self,count,mode,uid):
        #获取解码后原地址
    ...
    def download_all(self,count,author_list,video_list,aweme_id,nickname,dynamic_cover,mode,save):
        for i in range(count):
            ...
            try:
                ...
                print('音频',music_title,'    下载中')
                with open(save + mode + "\\" + nickname[i] + '\\' + music_title + '.mp3','wb') as f:
                    f.write(r.content)
            except:
                if music_url == '':
                    print('该音频目前不可用\r')
...
"""
    ...
    #返回个人主页api数据
    result = TiTk.get_info(count,mode,uid)
    #处理视频api数据
